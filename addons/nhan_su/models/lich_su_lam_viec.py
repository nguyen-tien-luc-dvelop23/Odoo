...
class LichSuLamViec(models.Model):
    ...
    loai_lich_su = fields.Selection([
        ('chuc_vu', 'Thay đổi Chức vụ / Phòng ban'),
        ('du_an', 'Tham gia Dự án'),
        ('cong_viec', 'Thực hiện Công việc')
    ], string="Loại Lịch sử", default='chuc_vu')

    du_an_id = fields.Many2one('du_an', string="Dự án liên quan", ondelete='cascade')
    cong_viec_id = fields.Many2one('cong_viec', string="Công việc liên quan", ondelete='cascade')

    ngay_bat_dau = fields.Date(string="Ngày bắt đầu", default=fields.Date.context_today)
